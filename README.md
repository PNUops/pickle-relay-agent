...
- `PICKLE_RELAY_NEW_CONN_BURST=0`은 "버스트 없음"이 아니라 커널이 **5패킷 허용치**로 보정한다
  (netlink 직행이라 nft CLI 기본 5와 동일). 대역은 릴레이 서비스 포트를 포함할 수 없다 —
  에이전트가 2222(admin sshd)·51820(WireGuard)을 포함한 대역을 기동 시 거부한다(:22는 하한 1024 미만).
